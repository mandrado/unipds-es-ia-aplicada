...
async function executeQuery(query: string | undefined, neo4jService: Neo4jService) {
  try {
    if (!query || !query.trim()) {
      return {
        results: null,
        error: 'No Cypher query generated for this question',
      };
    }
  ...
  return async (state: GraphState): Promise<Partial<GraphState>> => {
    try {
      ...
      if (error && results === null) {
        if (error === 'No Cypher query generated for this question') {
          return {
            ...state,
            error,
            needsCorrection: false,
